...
firstHalf xs = take (div (len + 1) 2) xs
    where len = length xs
...
capitalize xs = unwords $ map (\x -> (toUpper $ head x) : tail x) $ words xs
...
powers :: Int -> Int -> [Int]
-- powers k maxVal = takeWhile (<= maxVal) (iterate (*k) 1)
powers k maxVal = [k ^ n | n <- takeWhile ((<=maxVal) . (k^)) [0..maxVal]]
...
joinToLength len xs = [s1 ++ s2 | s1 <- xs, 
                                  s2 <- xs, 
                                  let l1 = length s1; l2 = length s2 in l1 + l2 == len ]
...
multiCompose fs x = multiCompose' fs x
multiCompose' fs = case fs of
    [] -> id
    (f:ft) -> (.) f  $ multiCompose' ft
...
multiApp' gs x = case gs of
    [] -> []
    (g:gt) -> (g x) : multiApp' gt x
...
interpreter' commands x y = case commands of
    [] -> []
    (c:ct) -> case c of
        "up" -> interpreter' ct x (y + 1)
        "down" -> interpreter' ct x (y - 1)
        "left" -> interpreter' ct (x - 1) y
        "right" -> interpreter' ct (x + 1) y
        "printX" -> show x : (interpreter' ct x y)
        "printY" -> show y : (interpreter' ct x y)
